...
---
# <a name="how-to-generate-xml-from-csv-files-linq-to-xml"></a>CSV ファイルから XML を生成する方法 (LINQ to XML)

C# または Visual Basic の LINQ to XML を使用し、コンマ区切り値 (CSV) ファイルから XML を作成できます。

## <a name="example-generate-xml-from-a-csv-file"></a>例: CSV ファイルから XML を生成する

この例では、文字列の配列に LINQ クエリを実行し、`let` 句を使用して各文字列をフィールドの配列に分割します。
