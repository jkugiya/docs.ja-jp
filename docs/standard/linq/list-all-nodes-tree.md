...
---
# <a name="how-to-list-all-nodes-in-a-tree-linq-to-xml"></a>ツリー内のすべてのノードを一覧表示する方法 (LINQ to XML)

ツリー内のすべてのノードを一覧表示すると、メソッドやプロパティがツリーに対してどのように影響を与えているかを正確に把握したい場合などに便利です。 次の C# および Visual Basic の例では、ツリーのノードを一覧表示します。 各ノードは、ツリー内のノードの場所を指定する XPath 式で表されます。

> [!NOTE]
> LINQ to XML を使用して XPath 式を実行する場合は、特に便利ではありません。 XPath 式よりも、LINQ to XML クエリの方がパフォーマンスが高く、はるかに強力です。 ただし、XPath 式は、XML ツリー内のノードを識別する手段としては有効です。

## <a name="example-use-xpath-to-list-all-nodes-in-a-tree"></a>例: XPath を使用してツリー内のすべてのノードを一覧表示する

この例では、XML ツリー内の任意のノードに対して特定の XPath 式を生成する、`GetXPath` という名前の関数を示します。 これは、ノードが名前空間内にある場合にも当てはまります。 生成された XPath 式の要素には、必要に応じて名前空間プレフィックスがあります。

この例では、複数の種類のノードを含む小さな XML ツリーを作成します。 次に、ノードを反復処理し、各ノードに対する XPath 式を出力します。

XML 宣言は、ツリー内のノードではありません。

入力として機能する XML ツリーを次に示します。
...
XPath 式によって定義される、XML ツリー内のノードの一覧を次に示します。
