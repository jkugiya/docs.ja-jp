...
description: (例で使用されている) この XML ファイルには、顧客と注文に関するデータが含まれています。
ms.date: 07/20/2015
...
ms.locfileid: "103412063"
---
# <a name="sample-xml-file-customers-and-orders-linq-to-xml"></a><span data-ttu-id="f92d5-103">サンプル XML ファイル: 顧客と注文 (LINQ to XML)</span><span class="sxs-lookup"><span data-stu-id="f92d5-103">Sample XML file: Customers and orders (LINQ to XML)</span></span>

<span data-ttu-id="f92d5-104">次の XML ファイルは、LINQ to XML ドキュメントのさまざまな例で使用されます。</span><span class="sxs-lookup"><span data-stu-id="f92d5-104">The following XML file is used in various examples in the LINQ to XML documentation.</span></span> <span data-ttu-id="f92d5-105">このファイルには顧客と注文が含まれています。</span><span class="sxs-lookup"><span data-stu-id="f92d5-105">This file contains customers and orders.</span></span>

<span data-ttu-id="f92d5-106">「[サンプル XSD ファイル: 顧客と注文](sample-xsd-file-customers-orders.md)」の記事には、このドキュメントの検証に使用できる XSD が含まれています。</span><span class="sxs-lookup"><span data-stu-id="f92d5-106">The article [Sample XSD file: Customers and orders](sample-xsd-file-customers-orders.md) contains an XSD that can be used to validate this document.</span></span> <span data-ttu-id="f92d5-107">この XSD は、XSD の `xs:key` 機能と `xs:keyref` 機能を使用して、`CustomerID` 要素の `Customer` 属性がキーであることを確立し、各 `CustomerID` 要素の `Order` 要素と各 `CustomerID` 要素の `Customer` 属性の間にリレーションシップを確立します。</span><span class="sxs-lookup"><span data-stu-id="f92d5-107">It uses the `xs:key` and `xs:keyref` features of XSD to establish that the `CustomerID` attribute of the `Customer` element is a key, and to establish a relationship between the `CustomerID` element in each `Order` element and the `CustomerID` attribute in each `Customer` element.</span></span>

<span data-ttu-id="f92d5-108">`Join` 句を使用してこのリレーションシップを利用する LINQ クエリの作成例については、「[2 つのコレクションを結合する方法](join-two-collections.md)」を参照してください。</span><span class="sxs-lookup"><span data-stu-id="f92d5-108">For an example of writing LINQ queries that take advantage of this relationship using the `Join` clause, see [How to join two collections](join-two-collections.md).</span></span>

## <a name="customersordersxml"></a><span data-ttu-id="f92d5-109">CustomersOrders.xml</span><span class="sxs-lookup"><span data-stu-id="f92d5-109">CustomersOrders.xml</span></span>
