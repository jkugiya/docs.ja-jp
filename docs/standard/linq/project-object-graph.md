...
---
# <a name="how-to-project-an-object-graph-linq-to-xml"></a>オブジェクト グラフを射影する方法 (LINQ to XML)

C# または Visual Basic を使用して、XML からオブジェクト グラフを射影または設定できます。

## <a name="example-populate-an-object-graph-from-xml"></a>例: XML からオブジェクト グラフを設定する

この例では、「[サンプル XML ファイル: 一般的な購買発注書](sample-xml-file-typical-purchase-order.md)」の XML ドキュメントから、`Address`、`PurchaseOrder`、`PurchaseOrderItem` の各クラスを使用して、オブジェクト グラフを設定します。
