...
クラス '\<classname>' の 'Microsoft.VisualBasic.ComClassAttribute' は、\<type> '\<typename>' にある同じ名前のメンバーと競合する \<type> '\<membername>' を暗黙的に宣言します。 ベース '\<typename>' の名前を非表示にする場合は、'Microsoft.VisualBasic.ComClassAttribute(InterfaceShadows:=True)' を使用します。  
